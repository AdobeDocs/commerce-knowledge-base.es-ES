...
description: Este artículo proporciona soluciones para solucionar discrepancias en los datos de Magento BI. Las exportaciones de datos son una herramienta útil para comparar los datos de Magento BI con los datos de origen a fin de identificar discrepancias de datos en los informes, especialmente si la [lista de comprobación de diagnóstico de discrepancias de datos](https://experienceleague.adobe.com/es/docs/experience-cloud-kcs/kbarticles/ka-41479) no le ayudó a identificar el problema. Este artículo le guiará por un ejemplo en la vida real de cómo se pueden identificar las discrepancias de datos mediante las exportaciones de datos.
...
Este artículo proporciona soluciones para solucionar discrepancias en los datos de Magento BI. Las exportaciones de datos son una herramienta útil para comparar los datos de Magento BI con los datos de origen con el fin de detectar discrepancias de datos en los informes, especialmente si la [lista de comprobación de diagnóstico de discrepancias de datos](https://experienceleague.adobe.com/es/docs/experience-cloud-kcs/kbarticles/ka-41479) no le ayudó a identificar el problema. Este artículo le guiará por un ejemplo en la vida real de cómo se pueden identificar las discrepancias de datos mediante las exportaciones de datos.
...
Si ambos sistemas tienen el mismo recuento de filas y la métrica **Ingresos** no coincide con los datos de origen, el **pedido\_total** debe estar desactivado en algún lugar. Es posible que el campo **order\_total** se haya actualizado en la base de datos de origen y Magento BI no esté recogiendo estos cambios.

Para confirmarlo, compruebe de nuevo si la columna **order\_total** se está comprobando o no. Vaya a Data Warehouse Manager y haga clic en la tabla **`orders`**. Verá la [frecuencia de repetición](https://experienceleague.adobe.com/docs/commerce-business-intelligence/mbi/analyze/warehouse-manager/cfg-data-rechecks.html?lang=es) en la columna &#39;¿Cambios?&#39;. El campo **pedido\_total** debe configurarse para que vuelva a comprobarse con la frecuencia con la que se espera que cambie; si no lo es, continúe y configúrelo en la frecuencia de comprobación que desee.
...
## La base de datos de origen tiene MÁS filas que Magento BI {#morerows}

Si la base de datos de origen tiene más filas que Magento BI y el espacio es mayor que el número de pedidos que puede esperar recibir durante la duración de un ciclo de actualización, puede haber un problema de conexión. Esto significa que Magento BI no puede extraer nuevos datos de la base de datos de origen, lo que puede ocurrir por varios motivos.
...
1. **Si el estado es Error** , es posible que la conexión no se haya configurado correctamente en el servidor. Las conexiones fallidas suelen proceder de un nombre de host incorrecto o de que el servidor de destino no acepte conexiones en el puerto especificado.Haga clic en la conexión y vuelva a comprobar la ortografía del nombre de host y que se ha introducido el puerto correcto. En el lado del servidor, asegúrese de que el puerto pueda aceptar conexiones y de que el firewall tenga permitida la dirección IP de Magento BI (54.88.76.97/32). **Si la conexión continúa fallando** , consulte la [sección Contacto con soporte técnico](#support) al final de este artículo para ver los pasos siguientes.
...
En el Administrador de Data Warehouse, las columnas de clave principal se marcan con un símbolo de clave. En nuestro ejemplo, la clave principal es la columna **order\_id**:
...
* **Si la base de datos de origen tiene MÁS filas que Magento BI** y la conexión se muestra como Correcta o continúa fallando, necesitaremos saber el nombre de la conexión y el mensaje de error que está viendo, si hay uno.
* **Si la base de datos de origen tiene MENOS filas que Magento BI,** filas no se eliminan de la tabla y las frecuencias de repetición de comprobación se han establecido correctamente, realice una búsqueda VLOOKUP en la hoja de cálculo **para encontrar qué valores order\_id están en Magento BI**, pero no en la base de datos de origen. Incluya estos valores cuando envíe su solicitud.

## Lectura relacionada

* [Lista de comprobación de diagnóstico de discrepancias de datos](https://experienceleague.adobe.com/es/docs/experience-cloud-kcs/kbarticles/ka-41479)
